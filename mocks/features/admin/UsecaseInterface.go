...
// UpdateUserAdmin provides a mock function with given fields: input, role, file, fileheader, err
func (_m *UsecaseInterface) UpdateUserAdmin(input admin.UserCore, role string, file multipart.File, fileheader *multipart.FileHeader, err error) (admin.UserCore, error) {
	ret := _m.Called(input, role, file, fileheader, err)

	var r0 admin.UserCore
	if rf, ok := ret.Get(0).(func(admin.UserCore, string, multipart.File, *multipart.FileHeader, error) admin.UserCore); ok {
		r0 = rf(input, role, file, fileheader, err)
	} else {
		r0 = ret.Get(0).(admin.UserCore)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(admin.UserCore, string, multipart.File, *multipart.FileHeader, error) error); ok {
		r1 = rf(input, role, file, fileheader, err)
	} else {
		r1 = ret.Error(1)
	}
